...
export NWNX_CORE_LOAD_PATH=/mnt/d/Development/ubutnu/unified/Binaries
...
LD_PRELOAD=/mnt/d/Development/ubutnu/unified/Binaries/NWNX_Core.so \
 ./nwserver-linux \
  -module "$MODNAME" \
  -maxclients 32 \
  -minlevel 1 \
  ...
  -playerpassword '' \
  -dmpassword '' \
  -servername 'InfyDae-Test Server' \
  -publicserver 0 \
  -reloadwhenempty 0 \
  -port 5121
